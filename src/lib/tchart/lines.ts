...
export default class TLines {
	...
	onResize() {
		...
		const dims = this.opts.additional.mini
	...
	render() {
		...
		const toCache =
			mini ||
			(opts.data.master &&
				state.masterVisibility < 1 &&
				state.masterVisibility > 0) ||
			(opts.data.slave &&
				state.slaveVisibility < 1 &&
				state.slaveVisibility > 0);
		...
		xScale =
			(dims.w - pRight - pLeft) /
			(x2 - x1 + (isStepMode ? this.opts.data.mainPeriodLen * (1 - morph) : 0));
		...
		if (toCache) {
			const hash = [
				dims.w,
				dims.h,
				mini ? state.xg1 : state.x1,
				mini ? state.xg2 : state.x2,
				this.isDarkMode,
				zoom,
			];
			if (!mini) {
		...
		const lineWidth =
			(opts.additional.mini
				? 1
				: opts.data.strokeWidth === "auto"
					? ysLen > 5
						? 1
						: 2
		...
		for (i = 0; i < ysLen; i++) {
			...
			for (j = xInd1; j <= xInd2; j++) {
				...
				if (xc > prevXc || (isStepMode && j === d2 + 1)) {
					...
					isStepMode &&
						ctx.lineTo(
							((x[j] * xScale + xw + xShift) << 0) + lineWidthShift,
							yc - lineWidthShift,
						);
				} else {
