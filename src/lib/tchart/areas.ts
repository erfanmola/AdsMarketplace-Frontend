...
import type { TChartAngle, TChartUnitOptions } from "./types";
...
export default class TAreas {
	...
	onResize() {
		if (this.opts.additional.mini) {
			...
			var dims = this.opts.additional.mini
				? this.opts.state.dims.mini
				: this.opts.state.dims.graph;
	...
	render() {
		...
		if (zoomMorph > 0 && !mini) {
			...
			if (morph < 1) {
				...
				xInd1Real = getXIndex(
					x,
					this.opts.state.zoomDir === -1 ? this.savedX1 : x1End,
					true,
				);
				xInd2Real = getXIndex(
					x,
					this.opts.state.zoomDir === -1 ? this.savedX2 : x2End,
					true,
				);
			} else {
			...
			for (i = 0; i < colsLen; i++) {
				...
				tmp =
					(xInd1RealCeil - xInd1Real) *
					(tmpY[xInd1RealFloor] || 0) *
					opacityCols[i];
				totalPerItem[i] += tmp;
				totalForAll += tmp;
				...
				tmp =
					(xInd2Real - xInd2RealFloor) *
					(tmpY[xInd2RealCeil] || 0) *
					opacityCols[i];
				totalPerItem[i] += tmp;
				totalForAll += tmp;
			}
			...
			const elastic =
				this.opts.state.zoomDir === 1
					? Math.min(
							Math.max(
								morph < 0.85 ? (morph - 0.65) / 0.2 : 1 - (morph - 0.9) / 0.15,
								0,
							),
							1,
						) ** 0.8
			...
			for (i = 0; i < colsLen; i++) {
				...
				const additionalPoints = Math.round(
					percentage * rLen * pointsPerArcLen,
				);
				...
				angles.push({
					...
					percentage:
						percentage === 0 ? 0 : Math.max(Math.round(percentage * 100), 1),
					percentageText:
						percentage === 0
							? ""
							: percentage < 0.01
		...
		for (i = 0; i < ysLen; i++) {
			...
			if (zoomMorph === 0 || !mini) {
				...
				if (zoomMorph === 0) {
					if (i > 0) {
						ctx.moveTo(
							(optX[xInd2] * xScale + xShift) << 0,
							(hBottom - prevY[xInd2] + overlap) << 0,
						);
						for (j = xInd2 - 1; j >= xInd1; j--) {
							ctx.lineTo(
								(optX[j] * xScale + xShift) << 0,
								(hBottom - prevY[j] + overlap) << 0,
							);
						}
					} else {
					...
					const calcTrans = (
						...
						const fromR =
							((cy - fromY) * (cy - fromY) + (fromX - cx) * (fromX - cx)) **
							0.5;

						if (
							Math.abs(toAngle - fromAngle) >
							Math.PI * (colsLen === 1 ? 1.5 : 1)
						) {
							toAngle -= Math.PI * 2;
						}
					...
					const additionalSteps =
						zoomMorph < 1 ? 4 : angles[colInd].additionalPoints;
					let dist: number;
					let cBot = false,
						cTop = false,
						xj: number;
					...
					if (colInd < colsLen - 1) {
						...
						for (j = 0; j <= additionalSteps; j++) {
							...
							res = calcTrans(
								...
								angles[colInd].st +
									(j / additionalSteps) *
										(angles[colInd].ed - angles[colInd].st),
								radius,
							);
						...
						for (j = 0; j <= additionalSteps; j++) {
							res = calcTrans(
								(optX[xInd1] +
									(j / additionalSteps) * (optX[xInd2] - optX[xInd1])) *
									xScale +
									xShift,
								0,
								angles[colInd].st +
									(j / additionalSteps) *
										(angles[0].st - 2 * Math.PI - angles[colInd].st),
								radius,
							);
				...
				if (i > 0) {
					...
					for (j = xInd2; j >= xInd1 + 1; j--) {
						ctx.lineTo(
							optX[j] * xScale + xShift,
							hBottom -
								(prevY[j] + morph * (prevY[j - 1] - prevY[j])) +
								overlap,
						);
						ctx.lineTo(
							optX[j - 1] * xScale + xShift,
							hBottom - prevY[j - 1] + overlap,
						);
					}
				} else {
				...
				if (colInd < colsLen - 1) {
					for (j = xInd1; j <= xInd2 - 1; j++) {
						...
						if (j === xInd2 - 1) {
							ctx.lineTo(
								optX[xInd2] * xScale + xw + xShift,
								hBottom - prevY[xInd2] - curHNext,
							);
						}
			...
			if (!mini && zoomMorph > 0 && angles[colInd].percentageText) {
				const opacity =
					morph ** (this.opts.state.zoomDir === 1 ? 4 : 20) *
					o *
					(state[`f_${i}`] * 0.9 + 0.1);
				let fontSize = Math.max(
					Math.min(angles[colInd].percentage * 2, 26),
					10,
				);
				...
				const isOutboard =
					angles[colInd].percentage < opts.data.pieLabelsPercentages.outboard;
				...
				if (
					angles[colInd].percentage < opts.data.pieLabelsPercentages.hoverOnly
				) {
				...
				if (isOutboard) {
					...
					const lx2 =
						cx + sx + cosVal * (rad + 6 * (1 - selectionOffset) - 1) * dpi;
					const ly2 =
						cy + sy - sinVal * (rad + 6 * (1 - selectionOffset) - 1) * dpi;
				...
				const dx =
					cosVal * offset * (fullyVisibleInd === colInd ? textToCenter : 1);
				const tx =
					cx +
					sx +
					dx * dpi +
					(isOutboard
						? (((fontSize / 4) * angles[colInd].percentageText.length * dx) /
								offset) *
							dpi
						: 0);
				const ty =
					...
					sinVal *
						offset *
						(fullyVisibleInd === colInd ? textToCenter : 1) *
						dpi;

				ctx.font = `${opts.settings.FONT.bold} ${fontSize * dpi}px ${opts.settings.FONT.family}`;
				ctx.fillText(
					angles[colInd].percentageText,
					tx,
					ty + (fontSize * dpi) / 2.9,
				); // fontSize * dpi / 2.9 cause text render point is baseline
