...
import { getFormatter, type yTickFormatter } from "./utils";
...
export default class TAxisY {
	...
	render(opacity: number) {
		let calcDataLeft: ReturnType<TAxisY["calcAxisData"]>,
			calcDataRight: ReturnType<TAxisY["calcAxisData"]>;
		if (this.opts.pairY) {
			...
			if (
				(calcDataRight.needAnimation && !calcDataLeft.needAnimation) ||
				this.opts.state["o_0"] < 1
			) {
	...
	calcAxisData(y1Name: "y1" | `y1_${number}`, y2Name: "y2" | `y2_${number}`) {
		...
		const changeSpeedFirst =
			state[y1Name] > y1 ? state[y1Name] / y1 : y1 / state[y1Name];
		const changeSpeedLast =
			state[y2Name] > y2 ? state[y2Name] / y2 : y2 / state[y2Name];

		const yEndRange = y2 - y1;
		const yScaleCur =
			(this.opts.state.dims.axisYLines.h - pTop - pBottom) / yCurRange;
		const yScaleEnd =
			(this.opts.state.dims.axisYLines.h - pTop - pBottom) / yEndRange;
	...
	updateAxisState(
		...
		for (let i = 0; i <= linesCount; ++i) {
			...
			const numRealLeft =
				leftData.yRealStart + Math.round(leftData.yRealStep * i);
			const numRealRight =
				rightData.yRealStart + Math.round(rightData.yRealStep * i);
			const formatter = getFormatter(
				"yTickFormatter",
				opts.data,
				state.zoomMorph,
			) as any as typeof yTickFormatter;
			...
			const numDisplayRightStr = formatter(
				Math.max(numRealRight, 0),
				rightData.yRealStep,
				true /* , this.opts.state['e_0']*/,
			);
			...
			if (baseData.needAnimation) {
				const oldFrom =
					...
					dims.h -
					pBottom -
					(numReal - baseData.y1) * baseData.yScaleEnd;
				const oldTo =
					dims.t +
					dims.h -
					pBottom -
					(this.items[i][numName] - baseData.y1) * baseData.yScaleEnd;
				const newFrom =
					dims.t +
					dims.h -
					pBottom -
					(numReal - state[y1Name]) * baseData.yScaleCur;
				const newTo =
					dims.t +
					dims.h -
					pBottom -
		...
		for (const i in this.items) {
			...
			if (y - 6 >= 0 && y - 16 <= dimsLeft.h) {
				this.ctx.globalAlpha =
					o * (this.opts.pairY ? this.opts.state["o_0"] : 1) * opacity;
