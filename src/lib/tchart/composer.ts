...
import type { TChartAnimationProperty, TChartUnitOptions } from "./types";
...
export default class TComposer {
	...
	render(groups: TChartAnimationProperty["group"]) {
		if (this.deviceSpeed === undefined) {
			...
			this.deviceSpeed =
				(t2 - t1) /
				(this.opts.graphStyle === "line" || this.opts.graphStyle === "step"
					? 1.5
					: 2) /
				(this.opts.data.x.length * this.opts.data.ys.length);
	...
	renderInner(groups: TChartAnimationProperty["group"]) {
		...
		const pieChartAnimating =
			this.opts.graphStyle === "area" && state.zoomMode && zoomMorph < 1;
		const pieChartAnimated =
			this.opts.graphStyle === "area" && state.zoomMode && zoomMorph === 1;
		...
		if (this.opts.data.slave) {
			...
			this.opts.chart.$el.style.visibility =
				state.slaveVisibility > 0 ? "visible" : "hidden";
		}
		...
		if (groups.top) {
			...
			if (
				this.opts.graphStyle === "line" ||
				this.opts.graphStyle === "step" ||
				(this.opts.graphStyle === "area" && zoomMorph > 0) ||
				(this.opts.data.slave && state.slaveVisibility < 1) ||
				(this.opts.data.master && state.masterVisibility < 1)
			) {
				if (pieChartAnimated) {
		...
		if (groups.bottom) {
			...
			const isNotSpecialAndChangedSubchart =
				!this.opts.data.master &&
				!this.opts.data.slave &&
				this.opts.data.details &&
				this.opts.data.subchart.show !== this.opts.data.details.subchart.show;
			...
			if (isNotSpecialAndChangedSubchart) {
				if (zoomMorph > 0 && zoomMorph < 1) {
					...
					ctx.globalAlpha = this.opts.data.subchart.show
						? zoomMorph
						: 1 - zoomMorph;
					ctx.fillRect(
						dims.graph.l * dpi,
