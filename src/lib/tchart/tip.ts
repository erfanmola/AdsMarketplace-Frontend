...
export default class TTip {
	...
	onTipClick = (e: Event) => {
		...
		dataPromise.then(successDetailCallback).catch(errorDetailCallback);
	};
	...
	updateColors() {
		...
		this.labels.forEach((item, ind) => {
			this.points &&
				(this.points[ind].style.borderColor = this.isDarkMode
					? ys[ind].colors_n[0]
					: ys[ind].colors_d[0]);
			item.$value.style.color = this.isDarkMode
				? ys[ind].colors_n[2]
				: ys[ind].colors_d[2];
		});
	...
	getTp(x: number, y: number, isTouch?: boolean) {
		if (this.opts.graphStyle === "area" && this.opts.state.zoomMode) {
			...
			const cx = dims.w / 2;
			const cy = dims.h / 2;
			const dist = ((cy - y) * (cy - y) + (x - cx) * (x - cx)) ** 0.5;
	...
	toggle(enabled: boolean, shiftHide?: boolean) {
		...
		if (!enabled && this.shown) {
			...
			if (shiftHide) {
				...
				this.lastTipTop -= 12;
				this.lastTipLeft =
					this.lastTipLeft < this.opts.state.dims.tip.w / 2
						? this.lastTipLeft - 12
						: this.lastTipLeft + 12;
				this.$tip.style.transform =
					"translate(" + this.lastTipLeft + "px," + this.lastTipTop + "px)";
				this.$tip.style.webkitTransform =
					"translate(" + this.lastTipLeft + "px," + this.lastTipTop + "px)";
			}
	...
	renderPieTooltip(params?: any) {
		...
		this.pieLabel.$valueText.nodeValue = !isNaN(curPieItem.value)
			? formatter(curPieItem.value)
			: "n/a";
		...
		let top = Math.min(
			this.dy - tipMarginFromPointer - this.tipH,
			dimsTip.t + dimsTip.h - this.tipH - pBottom,
		);
		...
		left = Math.min(
			Math.max(left, pLeft / 2),
			dimsTip.w - this.tipW - pRight / 2,
		);
	...
	render(params: any = {}) {
		...
		const offsetForBarGraphMain =
			opts.graphStyle === "bar" || opts.graphStyle === "step"
				? this.opts.data.mainPeriodLen
				: 0;
		...
		const x =
			state.x1 +
			(state.x2 - state.x1 + offsetForBarGraphScale) *
				((constrainedDx - pLeft) / (dims.w - pRight - pLeft));
		...
		if (this.prevXInd !== xInd || !params.isMove) {
			...
			this.labels.forEach((item, ind) => {
				const display =
					opts.state[`e_${ind}`] && !isNaN(opts.data.ys[ind].y[xInd])
						? "block"
						: "none";
			...
			const lx =
				(opts.data.x[xInd] - state.x1 + xw / 2) /
				(state.x2 - state.x1 + offsetForBarGraphScale);
			const cx = (lx * (dims.w - pLeft - pRight) + pLeft) << 0;

			this.$line && (this.$line.style.transform = "translateX(" + cx + "px)");
			this.$line &&
				(this.$line.style.webkitTransform = "translateX(" + cx + "px)");
			...
			opts.data.ys.forEach((item, ind) => {
				if (opts.state[`e_${ind}`] && !isNaN(item.y[xInd])) {
					...
					if (this.points) {
						...
						$el.style.transform =
							"translateY(" +
							((dims.h - y * (dims.h - pTop - pBottom) - pBottom) << 0) +
							"px)";
			...
			if (!params.isMove) {
				...
				this.rowPaddings =
					parseInt(compStyles.getPropertyValue("padding-left"), 10) +
					parseInt(compStyles.getPropertyValue("padding-right"), 10);
			}
			...
			let minWidth =
				this.rowPaddings + this.maxLabelWidth + 20 + this.maxValueWidth; // 20 - min margin between label and value
		...
		if (pos === "center") {
			...
			top = Math.min(
				this.dy - tipMarginFromPointer - this.tipH,
				dims.t + dims.h - this.tipH - pBottom,
			);
			if (top < dims.t + pTop) {
				pos = "side";
			}
		...
		if (pos === "side") {
			...
			top = Math.min(
				Math.max(this.dy - this.tipH / 2, dims.t + pTop),
				dims.t + dims.h - this.tipH - pBottom,
			);
		}
	...
	fillPercentages(xInd: number, sumAll: number) {
		...
		opts.data.ys.forEach((item, ind) => {
			if (opts.state[`e_${ind}`]) {
				...
				this.labels[ind].$label.style.transform =
					"translateX(" + percentageWidth + "px)";
				this.labels[ind].$label.style.webkitTransform =
					"translateX(" + percentageWidth + "px)";
