...
export default class TFade {
	...
	onResize() {
		...
		const backgroundRgbJoined =
			this.opts.settings.COLORS.backgroundRgb.join(", ");

		if (this.opts.graphStyle !== "area") {
			const gradientTop = this.ctxFadeTop.createLinearGradient(
				0,
				0,
				0,
				dimsTop.h * dpi,
			...
			if (this.opts.graphStyle !== "bar") {
				const gradientBottom = this.ctxFadeBottom!.createLinearGradient(
					0,
					0,
					0,
					dimsBottom.h * dpi,
	...
	render() {
		...
		this.$fadeTop &&
			this.ctx.drawImage(this.$fadeTop, dimsTop.l * dpi, dimsTop.t * dpi);
		this.$fadeBottom &&
			this.ctx.drawImage(
				this.$fadeBottom,
				dimsBottom.l * dpi,
