...
export function triggerEvent(eventName: string, details: any) {
	...
	document.dispatchEvent(
		new CustomEvent(eventName, { detail: details || null }),
	);
}
...
export function drawRoundedRect2(
	ctx: CanvasRenderingContext2D,
	dpi: number,
	w: number,
	h: number,
...
export function getFormatter(
	...
	const lookupIn =
		zoomMorph && zoomMorph > 0.5 && data.details ? data.details : data;
	...
	if (f === "null") {
		const m: Record<string, Formatter> = {
			xRangeFormatter: (value) =>
				data.getLabelDate(value, { isShort: false, isMonthShort: false }),
...
export function yTickFormatter(
	val: number,
	step: number,
...
export function statsFormatKMBT(
	val: number,
	kmbt?: string,
	precision?: number,
) {
...
export function roundRange(
	...
	if (refRange) {
		const yd1 =
			(refRange.yMinOrig - refRange.yMin) /
			(refRange.yMaxOrig - refRange.yMinOrig);
		const yd2 =
			(refRange.yMax - refRange.yMaxOrig) /
			(refRange.yMaxOrig - refRange.yMinOrig);
		return {
			yMin: y1 - (y2 - y1) * yd1,
			yMax: y2 + (y2 - y1) * yd2,
	...
	let curPower = Math.max(10 ** Math.floor(Math.log10(step)), 1);
	...
	while (true) {
		...
		c++;
		if (c > 10) {
			// seems impossible? but have no time to prove it, so this is save exit from cycle
			return {
				yMinOrig: y1,
				yMaxOrig: y2,
				yMin: y1,
