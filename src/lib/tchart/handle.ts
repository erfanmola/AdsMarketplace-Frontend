...
import { drawRoundedRect, getElemPagePos, isTouchDevice } from "./utils";
...
export default class THandle {
				...
				this.tp = this.getTp(
					...
					dy -
						(this.opts.state.dims.composer.h -
							opts.settings.MINI_GRAPH_HEIGHT -
							opts.settings.MINI_GRAPH_BOTTOM),
					params.isTouch,
				);
				this.firstMove = true;
				return !this.tp;
	...
	getTp(x: number, y: number, isTouch: boolean) {
		...
		const xl1 =
			this.prevX1 + (isTouch ? (state.x1 === state.xg1 ? -5 : -15) : 0);
		let xl2 = xl1 + xw;
		...
		if (
			Math.abs(
				state.x2 -
					state.x1 -
					(zoomMode ? this.opts.data.mainPeriodLen : this.minRange),
			) < 0.01
		) {
	...
	onMouseMove = (e: MouseEvent) => {
		...
		const tp = this.getTp(
			...
			dy -
				(this.opts.state.dims.composer.h -
					this.opts.settings.MINI_GRAPH_HEIGHT -
					this.opts.settings.MINI_GRAPH_BOTTOM),
			false,
		);
