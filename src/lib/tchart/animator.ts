...
import type TComposer from "./composer";
import type {
	TChartAnimationItem,
...
export default class TAnimator {
	...
	private step = () => {
		...
		let item: TChartAnimationItem,
			...
			curVal: number,
			newVal: number;
		const group: TChartAnimationProperty["group"] = {
			top: false,
			bottom: false,
		};
		...
		let j = 0;
		while (j < done.length) {
			this.queue[done[j]].cbEnd &&
				this.queue[done[j]].cbEnd(this.queue[done[j]].state);
