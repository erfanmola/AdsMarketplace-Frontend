...
import type {
	TChartAnimationProperty,
	TChartConstructorOptions,
	TChartData,
	TChartDataDetails,
	TChartRange,
	TChartRangePaired,
...
export default class TChart {
	...
	constructor(opts: TChartConstructorOptions) {
		...
		this.data = {
			...
			pieLabelsPercentages: {
				outboard:
					opts.data.pieLabelsPercentages &&
					opts.data.pieLabelsPercentages.outboard !== undefined
						? opts.data.pieLabelsPercentages.outboard
						: 5,
				hoverOnly:
					opts.data.pieLabelsPercentages &&
					opts.data.pieLabelsPercentages.hoverOnly !== undefined
						? opts.data.pieLabelsPercentages.hoverOnly
						: 2,
			},
			subchart: {
				show:
					opts.data.subchart && opts.data.subchart.show !== undefined
						? opts.data.subchart.show
						: true,
		...
		opts.data.columns.forEach((item) => {
			...
			if (tp === "x") {
				...
				const xTooltipFormatter = getFormatter(
					"xTooltipFormatter",
					this.data,
					0,
				);
				const xTickFormatter = getFormatter("xTickFormatter", this.data, 0);
				const xRangeFormatter = this.data.subchart.show
					? getFormatter("xRangeFormatter", this.data, 0)
					: undefined;
	...
	reduceGlobalRange(params: TChartStateZoom) {
		...
		let minXInd = xg2Ind;
		let maxXInd = xg1Ind;
		let shift = this.state.zoomMode
			? this.graphStyle === "bar" || this.graphStyle === "step"
				? 1
				: 2
			: 0;
	...
	updateSpeed(speed?: number) {
		const points =
			this.state.activeColumnsCount *
			this.state.xCount *
			((this.state.x2 - this.state.x1) /
				(this.state.xMainMax - this.state.xMainMin)) **
				0.5;
	...
	getYMinMax(
		...
		const graphAreaWidth = this.state.dims
			? this.state.dims.graph.w
			: this.getGraphWidth(this.data.sideLegend).width;
		...
		let start = getXIndex(
			useSaved ? this.data.saved.x : this.data.x,
			x1 - datePerPixel * this.settings.PADD[3],
		);
		let end = getXIndex(
			useSaved ? this.data.saved.x : this.data.x,
			x2 + datePerPixel * this.settings.PADD[1],
		);
		...
		if (!useSaved && this.state.zoomMode) {
			// not backing up from zoom mode
		...
		const proceedMinMax = (
			...
			yMax: number,
			ind?: number,
			refRange?: ReturnType<typeof roundRange>,
		) => {
				...
				if (state[`e_${ind}`] || (ind === 0 && this.pairY)) {
					...
					if (this.graphStyle === "line") {
						if (y[floorStart] !== undefined && y[ceilStart] !== undefined) {
							yFirst =
								y[floorStart] +
								(start - floorStart) * (y[ceilStart] - y[floorStart]); // clipped part of line from the beginning
						...
						if (y[floorEnd] !== undefined && y[ceilEnd] !== undefined) {
							yLast =
								y[floorEnd] + (end - floorEnd) * (y[ceilEnd] - y[floorEnd]); // clipped part of line from the end
		...
		if (this.graphStyle === "bar") {
			...
			for (let i = floorStart; i <= ceilEnd; i++) {
				...
				for (let j = 0; j < colsLen; j++) {
					yCur +=
						(useSaved
							? this.data.saved.y[visibleCols[j]][i]
							: this.data.ys[visibleCols[j]].y[i]) || 0;
				}
	...
	onResize = () => {
		...
		this.state.dims = {
			...
			dates: {
				...
				h: s.DATES_HEIGHT,
				l:
					this.opts.settings.DATES_SIDE === "right"
						? rectGraph.width - s.DATES_WIDTH - s.PADD[1]
						: s.PADD[1],
				t: 0,
			},
			mini: {
	...
	createDOM(container: HTMLElement) {
		...
		this.data.sideLegend &&
			this.$switchers.classList.add("tchart--switchers__side-legend");
		this.$el.appendChild(this.$switchers);

		container.appendChild(this.$el);

		const rangeGraph = this.getYMinMax(
			this.state.x1,
			this.state.x2,
			false,
			true,
			...
			if (this.graphStyle === "line" || this.graphStyle === "step") {
				props.push({
					...
					state: this.state,
						...
						? (range as TChartRangePaired).min[i]
						: (range as TChartRangeSingle).min,
					duration: 500,
					fixed: true,
					tween: "exp",
					speed: 0.25,
			...
			if (this.graphStyle !== "area") {
				props.push({
					...
					state: this.state,
						...
						? (range as TChartRangePaired).max[i]
						: (range as TChartRangeSingle).max,
					duration: 500,
					fixed: true,
					tween: "exp",
					speed: 0.25,
	...
	onSwitcherChange = (enabled: boolean, ind: number) => {
		...
		if (typeof enabled !== "boolean") {
			...
			for (i = 0; i < this.data.ys.length; i++) {
				isAllOffExceptCurrent =
					isAllOffExceptCurrent &&
					(i === ind ? this.state[`e_${i}`] : !this.state[`e_${i}`]);
			}
		}
		...
		for (i = 0; i < e.length; i++) {
			...
			if (prevE[i] !== e[i]) {
				...
				props.push({
					...
					duration:
						this.graphStyle === "line" || this.graphStyle === "step"
							? 166
							: 300,
					delay:
						(e[i] && this.graphStyle === "line") || this.graphStyle === "step"
							? 200
							: 0,
					tween: "linear",
					group: {
			...
			if (this.graphStyle === "line" || this.graphStyle === "step") {
				props.push({
					...
					state: this.state,
						...
						? (rangeGraph as TChartRangePaired).min[i]
						: (rangeGraph as TChartRangeSingle).min,
			...
			if (this.graphStyle !== "area") {
				props.push({
					...
					state: this.state,
						...
						? (rangeGraph as TChartRangePaired).max[i]
						: (rangeGraph as TChartRangeSingle).max,
					duration: this.pairY ? 0 : 333,
					group: {
						top: true,
					},
			...
			if (this.graphStyle === "line" || this.graphStyle === "step") {
				props.push({
					...
					state: this.state,
						...
						? (rangeMini as TChartRangePaired).min[i]
						: (rangeMini as TChartRangeSingle).min,
			...
			if (this.graphStyle !== "area") {
				props.push({
					...
					state: this.state,
						...
						? (rangeMini as TChartRangePaired).max[i]
						: (rangeMini as TChartRangeSingle).max,
					duration: this.pairY ? 0 : 316,
					group: {
						bottom: true,
					},
	...
	toggleZoomSpecial(enabled: boolean, dt: number, details: TChartDataDetails) {
		...
		if (enabled) {
			var scale =
				(this.state.x2 - this.state.x1) /
				(this.state.dims.graph.w -
					this.settings.PADD[3] -
					this.settings.PADD[1]);
			var lPaddInDt = this.settings.PADD[3] * scale;
			var rPaddInDt = this.settings.PADD[1] * scale;
			this.state.zoomSpecialOrigin =
				(dt - this.state.x1 + lPaddInDt) /
				(this.state.x2 - this.state.x1 + lPaddInDt + rPaddInDt);
	...
	toggleZoom = (enabled: boolean, dt?: number, data?: TChartData) => {
		...
		if (data && this.specialZoomTransition === undefined) {
			if (data.columns.length !== this.data.ys.length + 1)
				this.specialZoomTransition = true;
		...
		if (this.specialZoomTransition) {
			...
			if (!this.slaveChart) {
				const clonedData: TChartConstructorOptions["data"] = JSON.parse(
					JSON.stringify(data),
				);
		...
		if (data) {
			data.subchart = {
				show:
					data.subchart && data.subchart.show !== undefined
						? data.subchart.show
						: true,
		...
		if (enabled) {
			...
			this.updateSpeed(
				this.zoomEnterSpeed / (this.graphStyle === "area" ? 2 : 1),
			);
		...
		this.state.xg1Ind = Math.floor(
			getXIndex(enabled ? this.data.x : this.data.saved.x, xg1),
		);
		this.state.xg2Ind = Math.ceil(
			getXIndex(enabled ? this.data.x : this.data.saved.x, xg2),
		);
	...
	insertDetails(xg1: number, xg2: number, details: TChartDataDetails) {
		...
		const xr1 =
			endMain -
			(this.graphStyle === "bar" || this.graphStyle === "step" ? 1 : 0); // not including it (for bars shoudl include)
		...
		for (let j = 0; j < this.data.ys.length; j++) {
			...
			for (let i = startDetail; i <= endDetail; i++) {
				...
				if (this.graphStyle === "bar") {
					...
					yInter =
						origY[fdx[i]] + (ndx[i] - fdx[i]) * (origY[cdx[i]] - origY[fdx[i]]);
				}
