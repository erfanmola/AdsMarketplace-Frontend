...
export default class TAxisX {
	...
	render(opacity: number) {
		...
		const isPointHasWidth =
			opts.graphStyle === "bar" || opts.graphStyle === "step";

		// fast calculation of average space occupied by label
		const space =
			(state.zoomMode && opts.data.details
				? opts.data.details.maxXTickLength
				: opts.data.maxXTickLength) * 9;

		const offsetForBarGraphMain = isPointHasWidth
			? this.opts.data.mainPeriodLen
			: 0;
		const offsetForBarGraphDetail = isPointHasWidth
			? this.opts.data.detailPeriodLen
			: 0;
		const offsetForBarGraph =
			offsetForBarGraphMain +
			(offsetForBarGraphDetail - offsetForBarGraphMain) * zoomMorph;
		const offsetForBarGraphScale = offsetForBarGraphMain * (1 - zoomMorph);

		const xStepMain =
			(dims.w - pRight - pLeft) /
			Math.round(
				(state.x2 - state.x1 + offsetForBarGraphMain) /
					this.opts.data.mainPeriodLen,
			);
		const xStepDetail =
			(dims.w - pRight - pLeft) /
			Math.round(
				(state.x2 - state.x1 + offsetForBarGraphDetail) /
					this.opts.data.detailPeriodLen,
			);
		const xStep = xStepMain + (xStepDetail - xStepMain) * zoomMorph;

		let skipEachMain = 2 ** Math.ceil(Math.log2(space / xStepMain));
		let skipEachDetail = 2 ** Math.ceil(Math.log2(space / xStepDetail));
		const lxScale =
			(dims.w - pLeft - pRight) /
		...
		const changeSpeed = this.prevXStep
			? this.prevXStep > xStepMain
				? this.prevXStep / xStepMain
				: xStepMain / this.prevXStep
			: 1;
		let k = 1 / changeSpeed ** 5;
		...
		const x2Start = Math.min(
			x2 + Math.ceil((pRight + space * 0.5) / xStep),
			x.length - 1,
		);
		...
		if (zoomMode) {
			const tmp1 = Math.max(
				x[this.opts.state.detailInd1],
				this.opts.state.xMainMin,
			);
			const tmp2 = Math.min(
				x[this.opts.state.detailInd2],
				this.opts.state.xMainMax,
			);
			dtOffset =
				Math.round((tmp2 - tmp1) / this.opts.data.mainPeriodLen) +
				(isPointHasWidth ? 0 : 1);
		}
		...
		for (let i = x1Start; i <= x2Start; i++) {
			...
			if (zoomMode) {
				if (i < this.opts.state.detailInd1) {
					...
					shown =
						Math.max(i - this.opts.state.detailInd1, 0) % skipEachDetail === 0;
					prefix = "d";
				} else {
					shown =
						Math.max(
							i -
								(this.opts.state.detailInd2 -
									this.opts.state.detailInd1 +
									1 -
									dtOffset),
							0,
						) %
			...
			if (item && item.state[`ox_${id}`] > 0) {
				const xc =
					(item.xi - state.x1 + offsetForBarGraph / 2) * lxScale + pLeft;
		...
		for (const i in this.items) {
			...
			if (
				item.tp === 1 &&
				(item.xi < state.x1 - pLeft / lxScale ||
					item.xi > state.x2 + pRight / lxScale)
			) {
				this.hideItem(i, k);
			}
		}
		...
		if (opts.data.datesRange[x1End] === opts.data.datesRange[x2End]) {
			...
			datesStr =
				opts.data.datesRange[x1End] + " — " + opts.data.datesRange[x2End];
		}
		...
		this.ctx.fillText(
			datesStr,
			(dimsDates.l + (opts.settings.DATES_SIDE === "right" ? dimsDates.w : 0)) *
				dpi,
